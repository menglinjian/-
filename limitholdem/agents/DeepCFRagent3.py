...
class DeepCFRagent():
    ...
    def deepCFR(self, i, Process_num=16):
        ...
        for t in range(0, self.CFR_num):
            ...
            print(len(self.regretMemory.memory))
    ...
    def traverse_tree(self, w, prob, player_id, regretMemory, policyMemory):
        ...
        try:
            ...
            if w==1:
                ...
                action_probs = np.ones(self.env.action_num)
        ...
        if current_player == player_id:
            try:
                ...
                if random.random()<self.sample_rate:
                    for action in legal_actions:
                        if w==1:
                            regrets[action] = (action_utilities[action] - state_utility[current_player])
                        else:
                            regrets[action] = max(0, self.regret_constrain(w-1)*regrets[action] + 
                                                    (action_utilities[action] - state_utility[current_player]))/self.regret_constrain(w)#*math.sqrt(w))
            ...
            try:
                ...
                for action in legal_actions:
                    ...
                    self.env.step_back()
                    state_utility += utility*action_prob
                '''
                ...
                if random.random()<self.sample_rate:
                    ...
                    if w==1:
                        ...
                        action_probs = (action_probs*prob*w + self.policy_constrain(w-1)*last_action_probs)/self.policy_constrain(w)
...
class CFREstimator():
    ...
    def train(self, Memory, iteration):
        ...
        if self.isRegret==True:
            ...
            optimizer = optim.Adam(self.Net.parameters(), lr=0.00001/math.log(iteration+1))
        for epoch in range(0,self.epochs):
            running_loss = 0.0
            for i in range(0,self.mini_batches):
                if i*self.batch_size<len(Memory.memory):
                    t, x_train, chips, y_train = Memory.sample(i*self.batch_size)
...
class Memory(object):
    ...
    def sample(self,index):
        ...
        samples = self.memory[index:index+self.batch_size]
