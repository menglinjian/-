...
class DeepCFRagent():
    def __init__(self, 
        ...
        #network
        self.regretNN = CFREstimator(self.env.action_num,epochs=epochs,mini_batches=mini_batches,isRegret=True,batch_size=batch_size)
        self.regretMemory = Memory(self.env.card2index, memory_size=memory_size, batch_size=batch_size)
        self.policyNN = CFREstimator(self.env.action_num, epochs=epochs, mini_batches=mini_batches,isRegret=False,batch_size=batch_size)
    ...
    def multi_traverse(self, q, player_id, num):
        ...
        for i in range(num):
            ...
            self.traverse_tree(self.iteration, 1, player_id, regretMemory, policyMemory)
        ...
        if current_player == player_id:
            try:
                ...
                if random.random()<self.sample_rate:
                    for action in legal_actions:
                        regrets[action] = max(0, (w-1)*regrets[action] + 
                                                action_utilities[action] - state_utility[current_player])/w
            ...
            try:
                for action in legal_actions:
                    ...
                    if random.random()<self.sample_rate:
                        ...
                        action_probs = action_probs*prob*w + last_action_probs
