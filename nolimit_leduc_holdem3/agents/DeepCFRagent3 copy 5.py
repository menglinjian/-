...
class DeepCFRagent():
    def __init__(self, 
                    ...
                    sample_rate = 0.5,
                    is_training = True, 
                    regret_memory_size = 20000, 
                    policy_memory_size = 6000, 
                    batch_size = 2, 
        ...
        self.regretMemory = Memory(self.env.card2index, memory_size=regret_memory_size, batch_size=batch_size)
        self.policyNN = CFREstimator(self.action_num, epochs=epochs, mini_batches=mini_batches,isRegret=False,batch_size=batch_size)
        self.policyMemory = Memory(self.env.card2index, memory_size=policy_memory_size, batch_size=batch_size)
    ...
    def normalize(self):
        self.regretMemory.normalize()
        self.policyMemory.normalize()

    def renormalize(self):
        self.regretMemory.renormalize()
        self.policyMemory.renormalize()
        ...
        if current_player == player_id:
            try:
                ...
                if random.random()<self.sample_rate:
                    for action in legal_actions:
                        if w!=0:
                            regrets[action] = max(0, (action_utilities[action] - state_utility[current_player]))
                        else:
                            regrets[action] = max(0, self.regret_constrain(w-1)*regrets[action] + 
                                                    (action_utilities[action] - state_utility[current_player]))/1#self.regret_constrain(w)#*math.sqrt(w))
            ...
            try:
                ...
                if random.random()<self.sample_rate:
                    ...
                    if w!=0:
                        ...
                        action_probs = (action_probs*prob*w*w + self.policy_constrain(w-1)*last_action_probs)/1#self.policy_constrain(w)
    ...
    def regret_constrain(self, t):
        try:
            if t<=0:
                return 1
            return self.regretMemory.norm[t-1]
        except Exception as e:
                print(1,str(e))
    ...
    def policy_constrain(self, t):
        try:
            if t<=0:
                return 1
            return self.policyMemory.norm[t-1]
        except Exception as e:
                print(2,str(e))
        '''
        if t<=0:
...
class CFREstimator():
    ...
    def train(self, Memory, iteration):
        ...
        if self.isRegret == True:
            optimizer = optim.Adam(self.Net.parameters(), lr=0.00001/math.log2(iteration+1))
        else:
            optimizer = optim.Adam(self.Net.parameters(), lr=0.00001/math.log2(iteration+1))
        for epoch in range(0,self.epochs):
            ...
            if (epoch+1)%50==0 or epoch == 0:
                if self.isRegret:
                    print('%dth epoch loss: %.3f' %(epoch + 1,1000*running_loss / self.mini_batches))
                else:
                    print('%dth epoch loss: %.3f' %(epoch + 1,1000*running_loss / self.mini_batches))
    ...
    def normalize(self):
        try:
            ...
            for item in self.memory:
                Max = max(Max, np.max(item[3]))
            self.norm.append(Max)  
            Max = self.norm[-1]
            self.memory = list(map(lambda x:[x[0],x[1],x[2],x[3]/Max],self.memory))
        except Exception as e:
            print(3,str(e))
