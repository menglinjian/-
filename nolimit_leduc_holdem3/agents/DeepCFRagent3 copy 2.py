...
class DeepCFRagent():
    def __init__(self, 
        ...
        self.policyMemory = Memory(self.env.card2index, memory_size=memory_size*2, batch_size=batch_size*2)
        ...
        if current_player == player_id:
            ...
            try:
                ...
                if random.random()<self.sample_rate:
                    ...
                    if w==1:
                        action_probs = action_probs*prob*self.action_num
                    else:
                        action_probs = (action_probs*prob*w*w*self.action_num + self.policy_constrain(w-1)*last_action_probs)/self.policy_constrain(w)
                    policyMemory.append([s, w, action_probs])

            except Exception as e:
                print(333, traceback.print_exception())
    ...
    def get_policy(self, state, legal_actions, isEval=False, isTest=False):
        x_test = self.state_to_array(state)
        chips = np.array(state['all_chips']).reshape(2,1)
