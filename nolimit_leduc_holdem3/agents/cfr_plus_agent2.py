...
class CFRAPlusgent():
    ...
    def __init__(self, env, isAbs=True, CFR_num=1, tra_num=10, model_path='./cfr_plus_model'):
        ''' Initilize Agent
    ...
    def feed(self, dicts, player_id, isRegret=True):
        if isRegret:
            for obs in dicts:
                self.regrets[obs] = dicts[obs]
        else:
            for obs in dicts:
                self.average_policy[obs] = dicts[obs]
            ...
            if current_player == player_id:
                try:
                    ...
                    if obs not in regretMemory:
                        regretMemory[obs] = np.zeros(self.env.action_num)

                    for action in legal_actions:
                        if obs in self.regrets:
                            regretMemory[obs][action] = max(0, self.regrets[obs][action] + 
                                                action_utilities[action] - state_utility[current_player])
                        else:
                            regretMemory[obs][action] = max(0, 0 + action_utilities[action] - state_utility[current_player])
                ...
                try:
                    if obs not in policyMemory:
                        policyMemory[obs] = np.zeros(self.env.action_num)
                    ...
                    for action in legal_actions:
                        ...
                        if obs in self.average_policy:
                            policyMemory[obs][action] = self.average_policy[obs][action] + w * prob * action_prob
                        else:
                            policyMemory[obs][action] = 0 + w * prob * action_prob
    ...
    def regret_matching(self, obs):
        ...
        if obs not in self.regrets:
            regret = np.zeros(self.env.action_num)
        else:
